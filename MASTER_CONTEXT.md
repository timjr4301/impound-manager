...
_Last updated: July 30, 2026 — sent+30 remnant cleanup: boot backfill rewrites stored PPI Letter-2 due dates to letter1.sent_date+30, plus the last delivery-anchored wording/logic swept out (vehicle-detail Task 3 card was still computing off delivery). Previous: Find Trucks VIN reclassification + release reconciliation (18 cars) + July-outage 1st-letter reconciliation from the UPS outbound export (56 letters created with real send dates → 68 tracking numbers attached → 61 delivered → 61 2nd-letter timers started). Memory: now on Render **Standard 2 GB** — the 512 MB OOM is RESOLVED. Parked: image backup (waiting on IT), relo-trans car categorization. Open: verify the fresh Towbook-synced car → 1st-letter queue handoff._
